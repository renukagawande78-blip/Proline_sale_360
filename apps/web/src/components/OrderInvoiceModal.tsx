...
interface OrderInvoiceModalProps {
  ...
  onClose: () => void;
  agencies?: Agency[];
  initialDocMode?: 'SALES_ORDER' | 'DISPATCH_CHALLAN';
}

export const OrderInvoiceModal: React.FC<OrderInvoiceModalProps> = ({ order, isOpen, onClose, agencies, initialDocMode }) => {
  if (!isOpen || !order) return null;

  // Primary Document Mode: 'SALES_ORDER' (Default) or 'DISPATCH_CHALLAN' (Invoice option removed per user request)
  const [docMode, setDocMode] = useState<'SALES_ORDER' | 'DISPATCH_CHALLAN'>(initialDocMode || 'SALES_ORDER');
  ...
  useEffect(() => {
    if (!order) return;
    if (initialDocMode) {
      handleModeChange(initialDocMode);
    } else {
      ...
      if (isBilledOrder) {
        handleModeChange('DISPATCH_CHALLAN');
      } else {
        handleModeChange('SALES_ORDER');
      }
    }
    const hasBeenBilled = Boolean(
      order.invoice_number || 
      order.status === 'BILLED' || 
      order.status === 'DISPATCHED' || 
      order.status === 'OUT_FOR_DELIVERY' || 
      order.status === 'COMPLETED' ||
      (order.billing_total_qty && order.billing_total_qty > 0)
    );
    setShowIssuedQty(hasBeenBilled);
  ...
  const getOrderPdfShareLink = () => {
    ...
    return `${baseUrl}?order_id=${encodeURIComponent(order.id || order.order_number)}&view=invoice&doc=${docMode.toLowerCase()}`;
  };

  const getShareSummaryText = () => {
    const docTitle = isChallan ? 'Delivery Dispatch Challan' : 'Sales Order Booking Form';
    const pdfLink = getOrderPdfShareLink();
    return [
      `📄 *${docTitle}* (Order #${order.order_number})`,
      `Party: *${order.agency_name}* | Brand: *${order.company_name}*`,
      ``,
  ...
  const handleShareEmail = () => {
    const docTitle = isChallan ? 'Delivery Challan' : 'Sales Order Form';
    const subject = `${docTitle} PDF Copy - ${order.order_number} (${order.agency_name})`;
    const body = `Please find the PDF copy link for Order #${order.order_number} (${order.agency_name}):\n\n${getOrderPdfShareLink()}`;
  ...
  const handleNativeShare = async () => {
    const pdfLink = getOrderPdfShareLink();
    const title = `${isChallan ? 'Delivery Challan' : 'Sales Order'} PDF - ${order.order_number}`;
    if (navigator.share) {
      try {
    ...
    // Fallback: Copy PDF Link to clipboard
    handleCopyPdfLink();
  };
            ...
            <div className="invoice-modal-actions" style={{ display: 'flex', gap: '0.45rem', alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              <button 
                ...
                style={{ 
                  ...
                  gap: '0.4rem', 
                  fontWeight: 800, 
                  background: copiedLink ? 'linear-gradient(135deg, #16a34a, #15803d)' : 'linear-gradient(135deg, #8b5cf6, #6d28d9)',
                  color: '#ffffff',
                  border: 'none',
                  borderRadius: 6,
                  cursor: 'pointer',
              ...
              >
                {copiedLink ? <Check size={15} /> : <Link2 size={15} />} {copiedLink ? 'Link Copied!' : 'Copy PDF Link'}
              </button>
        ...
        <div id="invoice-sheet" style={{ flex: 1, overflowY: 'auto', padding: '1.5rem', background: '#ffffff', color: '#000000', fontFamily: 'Inter, "Segoe UI", Arial, sans-serif' }}>
          ...
          <div style={{ border: '2px solid #000000', borderRadius: 2 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '3px 8px', fontSize: '0.675rem', fontWeight: 700, borderBottom: '1px solid #000000' }}>
              ...
              <span style={{ color: '#000000' }}>Generated by PROKAP</span>
            </div>
